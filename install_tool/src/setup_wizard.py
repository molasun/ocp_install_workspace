...
class SetupWizard:
    ...
    def scan_mirror_registry_images(self, arch: str) -> List[str]:
        """
        從 mirror-registry 二進位檔中搜尋需要的容器映像檔

        Args:
            arch: 架構，如 'amd64'

        Returns:
            映像檔清單，如 ['registry.redhat.io/quay/quay-rhel8:v3.12.18', ...]
        """
        tar_filename = f"mirror-registry-{arch}.tar.gz"
        tar_path = os.path.join(self.install_source_dir, tar_filename)
        if not os.path.exists(tar_path):
            log_error(f"找不到 mirror-registry tar: {tar_path}")
            return []

        tmp_dir = "/tmp/mirror-registry-scan"
        ...
        try:
            ...
            log_error(f"解壓 mirror-registry 失敗: {e}")
            return []

        binary_path = os.path.join(tmp_dir, "mirror-registry")
        if not os.path.exists(binary_path):
            log_error(f"找不到 mirror-registry 二進位檔: {binary_path}")
            return []
        ...
        try:
            ...
            log_error("找不到 strings 命令，請安裝 binutils")
            return []
        except Exception as e:
            log_error(f"strings 執行失敗: {e}")
            return []

        # 過濾出完整的映像檔引用（registry.redhat.io/repo/name:tag 格式）
        images = set()
        pattern = r'registry\.redhat\.io/[a-z0-9_-]+/[a-z0-9_-]+:v?[\w.-]+'
        ...
        log_info(f"掃描到 {len(sorted_images)} 個映像檔: {sorted_images}")
        return sorted_images

    def pull_and_save_image(self, image: str) -> Tuple[bool, str]:
        """
        拉取容器映像檔並儲存為 tar 檔到 install_source/

        Args:
            image: 映像檔完整名稱，如 'registry.redhat.io/quay/quay-rhel8:v3.12.18'

        Returns:
            (是否成功, tar 檔案路徑或錯誤訊息)
        """
        # 生成 tar 檔名（quay-rhel8:v3.12.18 → quay-rhel8-v3.12.18.tar）
        short_name = image.split('/')[-1].replace(':', '-')
        tar_name = f"{short_name}.tar"
        tar_path = os.path.join(self.install_source_dir, tar_name)

        # 若 tar 已存在則跳過
        if os.path.exists(tar_path) and os.path.getsize(tar_path) > 0:
            log_info(f"映像檔 tar 已存在，跳過: {tar_path}")
            return True, tar_path

        # podman pull
        log_info(f"拉取映像檔: {image}")
        try:
            result = subprocess.run(
                ["podman", "pull", image],
            ...
            return False, "找不到 podman 命令，請確認已安裝"
        except subprocess.TimeoutExpired:
            return False, f"拉取超時: {image}"
        except Exception as e:
            return False, f"拉取異常: {e}"

        if result.returncode != 0:
            return False, f"拉取失敗: {result.stderr[:200]}"

        # podman save
        log_info(f"打包映像檔: {tar_path}")
        try:
            result = subprocess.run(
            ...
            return False, f"儲存超時: {image}"
        except Exception as e:
            return False, f"儲存異常: {e}"

        if result.returncode != 0:
            return False, f"儲存失敗: {result.stderr[:200]}"

        log_success(f"映像檔已打包: {tar_path}")
        return True, tar_path
