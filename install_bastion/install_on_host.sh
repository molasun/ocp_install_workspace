...
# SUDO_USER for _get_real_home() to find ~/install_source
# HOME 保持 sudo 預設 (/root) — mirror-registry 安裝器用 $HOME 找 SSH key 目錄，
# 必須是 /root 才能直接寫入 /root/.ssh/authorized_keys
sudo SUDO_USER="$(whoami)" "${SCRIPT_DIR}/.venv/bin/streamlit" run \
